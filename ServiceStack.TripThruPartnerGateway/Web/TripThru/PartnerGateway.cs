...
namespace ServiceStack.TripThruGateway.TripThru
{
    public class Partner : Gateway
    {
        public new class GetPartnerInfo : Gateway.GetPartnerInfo
        {
            ...
            public override Response Get(Request request, RequestLog log = null)
            {
                ...
                try
                {
                    ...
                    log.Log("GetPartnerInfo called on " + partner.name + ": Response = " + response);
                    Logger.Log(log);
                    return response;
                }
                catch (Exception e)
        ...
        {
            public Partner partner;
            public Trip GetTrip(Request r, RequestLog log)
            {
                return new Trip(
                    partner: partner,
                    ID: r.tripID,
            ...
            public override Response Post(Request r, RequestLog log = null)
            {
                ...
                try
                {
                    ...
                    if (r.fleetID != null)
                    {
                        ...
                        {
                            Trip trip = GetTrip(r, log);
                            if (f.QueueTrip(trip, log))
                            {
                                Response response = new Response();
                                log.Log("DispatchTrip successful on " + partner.name + ", Response: " + response);
                                Logger.Log(log);
                                return response;
                            }
                        }
                    ...
                    }
                    {
                        ...
                        log.Log("DispatchTrip rejected on " + partner.name + ", no available drivers -- Response: " + response);
                        Logger.Log(log);
                        return response;
                    }
                }
        ...
        public new class QuoteTrip : Gateway.QuoteTrip
        {
            public Trip GetTrip(Request r, RequestLog log)
            {
                return new Trip(partner, null, Trip.Origination.Foreign, r.pickupLocation, r.pickupTime, log, r.paymentMethod, r.passengerID, r.passengerName, r.dropoffLocation,
            ...
            public override Response Get(Request r, RequestLog log)
            {
                log = Logger.CreateNewRequestLog();
                try
                {
                    ...
                    log.Log("QuoteTrip called on " + partner.name + ", Response: " + response + (pickupLocationNotServed ? " -- Pickup location is outside of coverage area" : ""));
                    Logger.Log(log);
                    return response;
                }
                catch (Exception e)
        ...
        public new class GetTripStatus : Gateway.GetTripStatus
        {
            ...
            public override Response Get(Request r, RequestLog log = null)
            {
                ...
                try
                {
                    ...
                    log.Log("GetTripStatus called on " + partner.name + ", Response: " + response);
                    Logger.Log(log);
                    return response;
                }
                catch (Exception e)
        ...
        public new class UpdateTripStatus : Gateway.UpdateTripStatus
        {
            ...
            public override Response Post(Request r, RequestLog log = null)
            {
                ...
                try
                {
                    ...
                    Trip t = partner.tripsByID[r.tripID];
                    Response response = new Response();
                    log.Log("UpdateTripStatus called on " + partner.name + ", Response: " + response);
                    log.Tab();
                    t.SetStatus(r.status, log, notifyPartner: false);
                    log.Untab();
                    Logger.Log(log);
                    return response;
                }
        ...
        {
            log.Log("Partner = " + name + ": SimRate = " + simInterval + ", StatusUpdateRate = " + updateInterval);
            log.Tab();
            log.Log("PartnerFleets:");
            log.Tab();
        ...
        public void GetTripStatusFromForeignServiceProvider(Trip trip, RequestLog log, bool force = false)
        {
            if (force || DateTime.UtcNow > trip.lastUpdate + updateInterval)
            {
                log.Log("Getting (Foreign) status of " + trip);
                log.Tab();
                ...
                if (response.status != null)
                    trip.SetStatus((Status)response.status, log, notifyPartner: false);
    ...
    public class Trip : IDName
    {
        ...
        public void SetStatus(Status value, RequestLog log, bool notifyPartner = true)
        {
            if (value == Status.Complete)
            {
                if (service == Origination.Foreign)
                {
                    ...
                    partner.DeactivateTrip(ID, Status.Complete, log, resp.price, resp.distance);
                }
                else
                    partner.DeactivateTrip(ID, Status.Complete, log, PartnerFleet.GetPrice(this), PartnerFleet.GetDistance(this));
            }
            else if (value == Status.Cancelled || value == Status.Rejected)
                partner.DeactivateTrip(ID, value, log);
            if (_status != value)
            {
                log.Log("Trip status changed from " + _status + " to " + value);
                _status = value;
                if ((origination == Origination.Foreign || service == Origination.Foreign) && notifyPartner)
                {
                    log.Log("Since trip originated elsewhere, notify originating partner through TripThru");
                    log.Tab();
        ...
        public Trip(Partner partner, string ID, Origination origination, Location pickupLocation, DateTime pickupTime, RequestLog log, PaymentMethod? paymentMethod = null, string passengerID = null, string passengerName = null, Location dropoffLocation = null,
           DateTime? dropoffTime = null, List<Location> waypoints = null, VehicleType? vehicleType = null, double? maxPrice = null, int? minRating = null, PartnerFleet fleet = null, Driver driver = null, string foreignID = null, TimeSpan? duration = null, double? price = null)
        {
            ...
            this.driver = driver;
            this.price = price;
            this.SetStatus(Status.Queued, log, notifyPartner: false);
        }
    }
    ...
    public class PartnerFleet : IDName
    {
        ...
        public void Log(RequestLog log)
        {
            log.Log("PartnerFleet = " + name + "(" + location + "): SimRate = " + simInterval + ", StatusUpdateRate = " + updateInterval + ", MaxTripAdvancedNotice = " + tripMaxAdvancedNotice + ", CriticalPeriod = " + criticalPeriod + ", MissedPeriod = " + missedPeriod + ", RemovalAge = " + removalAge + ", NumTripsPeriodHour = " + tripsPerHour);
            log.Tab();
            log.Log("Drivers:");
            log.Tab();
        ...
        public bool Dispatch(Trip t, RequestLog log)
        {
            if (!Serves(t.pickupLocation))
            {
                log.Log("Pickup location " + t.pickupLocation + " is outside of coverage area");
                return false;
            }
            ...
            if (availableDrivers.Count > 0)
            {
                ...
                log.Log("Dispatched to " + t.driver.name);
                t.SetStatus(Status.Dispatched, log);
                return true;
            }
            log.Log("No drivers are currently available");
            return false;
        }
        public void GenerateTrips()
        {
            ...
            for (int n = 0; n < numTripsToGenerate; n++)
            {
                ...
                log.Log("Pickup request (" + name + ") " + passenger.name + " requests to be picked up at " + route.start + " on " + pickupTime + " and dropped off at " + route.end);
                log.Tab();
                Trip trip = new Trip(
        ...
        public bool QueueTrip(Trip t, RequestLog log)
        {
                ...
                switch (t.status)
                {
                    case Status.Queued:
                    {
                        if (t.origination == Trip.Origination.Local)
                        {
                            ...
                            {
                                log.Log("Missed period reached: -- so cancel " + t);
                                log.Tab();
                                t.SetStatus(Status.Cancelled, log, notifyPartner: true);
                                log.Untab();
                                Logger.Log(log);
                                break;
                            }
                        ...
                        if (DateTime.UtcNow < t.pickupTime - criticalPeriod)
                            break;
                        log.Log("Ready for dispatch: " + t);
                        log.Tab();

                        if (!Dispatch(t, log) && t.origination == Trip.Origination.Local)
                        {
                            log.Tab();
                            ...
                            if (response.result == Gateway.Result.OK)
                            {
                                ...
                                log.Log("Trip was successfully dispatched through TripThru");

                            }
                            else
                                log.Log("Trip was rejected by TripThru");
                            log.Untab();

                        }
                        log.Untab();
                        Logger.CreateNewRequestLog();
                        break;
                    }
                    case Status.Dispatched:
                    {
                        if (t.service == Trip.Origination.Foreign)
                            partner.GetTripStatusFromForeignServiceProvider(t, log);
                        else if (DateTime.UtcNow >= t.pickupTime - t.PartnerFleet.GetDriverRoute(t.driver.location, t.pickupLocation).duration)
                        {
                            log.Log("Driver is now enroute: " + t);
                            log.Tab();
                            ...
                            t.driver.routeStartTime = DateTime.UtcNow;
                            t.SetStatus(Status.Enroute, log);
                            log.Untab();
                        }
                        else if (DateTime.UtcNow > t.lastUpdate + updateInterval)
                        {
                            log.Log("Getting status of: " + t);
                    ...
                    case Status.PickedUp:
                    {
                        ...
                        else
                        {
                            ...
                            {
                                log.Log("The destination has been reached for: " + t);
                                log.Tab();
                                t.dropoffTime = DateTime.UtcNow;
                                t.driver.PartnerFleet.CompleteTrip(t);
                                t.SetStatus(Status.Complete, log);
                                log.Untab();
                            }
                            else if (DateTime.UtcNow > t.lastUpdate + updateInterval)
                            {
                    ...
                    case Status.Complete:
                        {
                            if (t.dropoffTime == null)
                                partner.GetTripStatusFromForeignServiceProvider(t, log, true);

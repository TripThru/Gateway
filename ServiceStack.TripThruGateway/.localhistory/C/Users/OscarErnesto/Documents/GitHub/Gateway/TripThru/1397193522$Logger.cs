...
namespace Utils
{
    public class Logger
    {
        ...
        static RestRequest _restReq;
        static Dictionary<object, int> _numBegunRequests;
        static Dictionary<object, bool> _threadsEnabled; 
        ...
        public static void BeginRequest(string msg, object request, string tripId = null)
        {
            lock (Locker)
            {
                ...
                if (requestLog == null || (_threadsEnabled.ContainsKey(thread) && !_threadsEnabled[thread]))
                    return;
                if (!requestLog.ContainsKey(thread))
                {
                    ...
                    _numBegunRequests[thread] = 0;
                }

                if (!_numBegunRequests.ContainsKey(thread))
                    _numBegunRequests[thread] = 0;
                _numBegunRequests[thread] = _numBegunRequests[thread] + 1;
                _threadsEnabled[thread] = true;
                Log(msg, request);
                Tab();
            }
        }
        ...
        public static void EndRequest(object response)
        {
            ...
            {
                object thread = Thread.CurrentThread.ManagedThreadId;
                if (requestLog == null || !_threadsEnabled.ContainsKey(thread) ||
                    (_threadsEnabled.ContainsKey(thread) && !_threadsEnabled[thread]) ||
                    (_numBegunRequests.ContainsKey(thread) && _numBegunRequests[thread] == 0)
                )
                    return;
                ...
                if (_numBegunRequests[thread] == 0)
                {
                    ...
                    _numBegunRequests.Remove(thread);
                }
            }
        }
        ...
        public static void OpenLog(string id, string filePath = null, bool splunkEnabled = true)
        {
            ...
            lock (Locker)
            {
                ...
                _numBegunRequests = new Dictionary<object, int>();
                _threadsEnabled = new Dictionary<object, bool>();
                _restReq = new RestRequest();

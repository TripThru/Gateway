...
namespace Utils
{
    ...
    public class MapTools
    {
        ...
        {
            using (StreamReader sr = new StreamReader(locationAddresses_Filename))
            {
                var lines = sr.ReadToEnd();
                locationAddresses = JsonConvert.DeserializeObject<Dictionary<string, Pair<string, string>>>(lines);
                if (locationAddresses == null)
                    locationAddresses = new Dictionary<string, Pair<string, string>>();
            }
        ...
        private static void WriteGeoLocationAddresses()
        {
            if (locationAddresses_Filename != null)
            {
                ...
                using (StreamWriter sr = new StreamWriter(locationAddresses_Filename))
                {
                    var serializer = new JavaScriptSerializer();
                    string locationAddressesJson = JsonConvert.SerializeObject(locationAddresses);
        ...
        private static void WriteGeoLocationNames()
        {
            if (locationNames_Filename != null)
            {
                ...
                {
                    var serializer = new JavaScriptSerializer();
                    string locationNamesJson = JsonConvert.SerializeObject(locationNames);
        ...
        private static void WriteGeoRoutes()
        {
            if (routes_Filename != null)
            {
                ...
                {
                    string routesJson = JsonConvert.SerializeObject(routes);
                    sr.Write(routesJson);
                }
                /*
        ...
        private static List<Waypoint> IncreaseGranularity(int duration, int maxDuration, double totalDuration, double distance, double totalDistance, Location from, Location to)
        {
            List<Waypoint> wayPoints = new List<Waypoint>();

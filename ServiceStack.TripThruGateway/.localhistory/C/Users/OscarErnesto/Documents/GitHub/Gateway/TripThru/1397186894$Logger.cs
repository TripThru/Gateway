    ...
    public class Logger
    {
        public class SplunkClient
        {
            ...
            public SplunkClient()
            {
                ...
                this._sendThread = new Thread(new ThreadStart(SendThread));
                ...
                this._requestUrl = string.Format("1/inputs/http?index={0}&sourcetype=storm_multi_line&host={1}&source={2}",
                    ProjectId,
                    Environment.MachineName,
                    Source);

                if (!string.IsNullOrEmpty(TZ))
                    this._requestUrl += "&tz=" + TZ;
        ...
        public static void EndRequest(object response)
        {
            lock (Locker)
            {
                ...
                {
                    AddTag("Type", "INFO");
                    AddTag("Memory", (System.Diagnostics.Process.GetCurrentProcess().WorkingSet64 / 1048576).ToString() + "Mb");
                    var json = "";
                    if (response != null)

...
namespace ServiceStack.TripThruGateway
{
    public interface GatewayClient
    {
        ...
        Gateway.RegisterPartner.Response RegisterPartner(Gateway.RegisterPartner.Request request);
        Gateway.GetPartnerInfo.Response GetPartnerInfo(Gateway.GetPartnerInfo.Request request);
        Gateway.DispatchTrip.Response DispatchTrip(Gateway.DispatchTrip.Request request);
        Gateway.QuoteTrip.Response QuoteTrip(Gateway.QuoteTrip.Request request);
        Gateway.GetTripStatus.Response GetTripStatus(Gateway.GetTripStatus.Request request);
        Gateway.UpdateTripStatus.Response UpdateTripStatus(Gateway.UpdateTripStatus.Request request);
    }
    public class GatewayLocalClient : GatewayClient
    {
        ...
        public GatewayLocalClient(Gateway gateway)
        {
            this.gateway = gateway;
        }
        public GatewayLocalClient(string callbackURL)
        {
            this.callbackURL = callbackURL;
        }
        public Gateway.RegisterPartner.Response RegisterPartner(Gateway.RegisterPartner.Request request)
        {
            Logger.BeginRequest("RegisterPartner called by " + gateway.name, request);
            Gateway.RegisterPartner.Response response = gateway.registerPartner.Post(request);
            Logger.EndRequest(request);
            return response;
        }
        public Gateway.GetPartnerInfo.Response GetPartnerInfo(Gateway.GetPartnerInfo.Request request)
        {
            Logger.BeginRequest("GetPartnerInfo called by " + gateway.name, request);
            Gateway.GetPartnerInfo.Response response = gateway.getPartnerInfo.Get(request);
            Logger.EndRequest(request);
            return response;
        }
        public Gateway.DispatchTrip.Response DispatchTrip(Gateway.DispatchTrip.Request request)
        {
            Logger.BeginRequest("DispatchTrip called by " + gateway.name, request);
            Gateway.DispatchTrip.Response response = gateway.dispatchTrip.Post(request);
            Logger.EndRequest(request);
            return response;
        }
        public Gateway.QuoteTrip.Response QuoteTrip(Gateway.QuoteTrip.Request request)
        {
            Logger.BeginRequest("QuoteTrip called by " + gateway.name, request);
            Gateway.QuoteTrip.Response response = gateway.quoteTrip.Get(request);
            Logger.EndRequest(request);
            return response;
        }
        public Gateway.GetTripStatus.Response GetTripStatus(Gateway.GetTripStatus.Request request)
        {
            Logger.BeginRequest("GetTripStatus called by " + gateway.name, request);
            Gateway.GetTripStatus.Response response = gateway.getTripStatus.Get(request);
            Logger.EndRequest(request);
            return response;
        }
        public Gateway.UpdateTripStatus.Response UpdateTripStatus(Gateway.UpdateTripStatus.Request request)
        {
            Logger.BeginRequest("UpdateTripStatus called by " + gateway.name, request);
            Gateway.UpdateTripStatus.Response response = gateway.updateTripStatus.Post(request);
            Logger.EndRequest(request);
            return response;
        }
    }
    public class GatewayRestClient : GatewayClient
    {

...
namespace Simulator.Utils
{
    ...
    class MapTools
    {
        ...
        {
            using (CsvFileReader reader = new CsvFileReader("Geo-Location-Names.csv"))
            {
                CsvRow row = new CsvRow();
                while (reader.ReadRow(row, ','))
                {
            ...
            // load trip routes
            Dictionary<string, LinkedList<Waypoint>> routes = new Dictionary<string, LinkedList<Waypoint>>();
            using (CsvFileReader reader = new CsvFileReader("Geo-Routes.csv"))
            {
                CsvRow row = new CsvRow();
                while (reader.ReadRow(row, ','))
                {
        ...
        {
            using (CsvFileWriter writer = new CsvFileWriter("Geo-Routes.csv"))
            {
                int routeID = 0;
            ...
            using (CsvFileWriter writer = new CsvFileWriter("Geo-Location-Names.csv"))
            {

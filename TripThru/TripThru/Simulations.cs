...
namespace Simulator.Simulations
{
    ...
    {
        public Fleet fleet;
        public Passenger(string name, Fleet fleet = null)
            : base(name)
        {
            this.fleet = fleet;
        }
    ...
    public class Fleet : IDName
    {
        ...
        public Fleet(string name, Location location, List<Zone> coverage, List<Driver> drivers, List<VehicleType> vehicleTypes, 
            List<Pair<Location, Location>> possibleTrips, double costPerMile, double baseCost, double tripsPerHour, List<Passenger> passengers, Partner partner = null) : base(name)
        {
        ...
        public void Log()
        {
            Logger.Log("Fleet = " + name + "(" + location + "): SimRate = " + simInterval + ", StatusUpdateRate = " + updateInterval + ", MaxTripAdvancedNotice = " + tripMaxAdvancedNotice + ", CriticalPeriod = " + criticalPeriod + ", MissedPeriod = " + missedPeriod + ", RemovalAge = " + removalAge + ", NumTripsPeriodHour = " + tripsPerHour);
            Logger.Tab();
            Logger.Log("Drivers:");
            Logger.Tab();
                ...
                switch (t.status)
                {
                    case Status.Queued:
                    {
                        ...
                        if (!Dispatch(t))
                        {
                            ...
                            Gateway.DispatchTrip.Response response = partner.tripthru.dispatchTrip.Post(request);
                    ...
                    case Status.Dispatched:
                    {
                        ...
                        else if (DateTime.UtcNow >= t.pickupTime - t.fleet.GetDriverRoute(location, t.pickupLocation).duration)
                        {
                    ...
                    case Status.Enroute:
                    {
                        ...
                        else
                        {
                            ...
                            if (t.driver.location.Equals(t.pickupLocation))
                            {
                                ...
                                Logger.Tab();
                                t.driver.route = t.fleet.GetTripRoute(t.pickupLocation, t.dropoffLocation);
                                t.driver.routeStartTime = DateTime.UtcNow;
                    ...
                    case Status.PickedUp:
                    {
                        ...
                        {
                            Route route = t.fleet.GetTripRoute(t.pickupLocation, t.dropoffLocation);
                            t.driver.location = t.driver.route.GetCurrentWaypoint(t.driver.routeStartTime, DateTime.UtcNow);
    ...
    public class Partner : Gateway
    {
        ...
        public new class DispatchTrip : Gateway.DispatchTrip
        {
            ...
            public override Response Post(Request r)
            {
                if (r.fleetID != null)
                {
                    Fleet f = partner.fleets[r.fleetID];
        ...
        public Partner(Gateway tripthru, string name, List<Fleet> fleets = null)
        {
            this.tripthru = tripthru;
            this.ID = null; // will be populated upon registration.
            this.name = name;
        ...
        public void Log()
        {
            ...
            Logger.Log("Fleets:");
            Logger.Tab();
            foreach (Fleet f in this.fleets.Values)
                f.Log();
            Logger.Untab();
            Logger.Untab();
        }
